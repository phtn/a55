...
const ALL_TOKENS: readonly Token[] = ['usdc', 'ethereum', 'bitcoin', 'matic', 'usdt']
...
export const DEFAULT_ALLOWED_PAY_NETWORKS: readonly PayNetworkName[] = ['bitcoin', 'ethereum', 'polygon', 'sepolia']

const chainIdByNetwork = new Map<EvmNetworkName, number>(EVM_NETWORKS.map((network) => [network.name, network.chainId]))
...
export const getTokenFractionDigits = (token: Token | null): number => (token === 'usdc' || token === 'usdt' ? 6 : 8)
